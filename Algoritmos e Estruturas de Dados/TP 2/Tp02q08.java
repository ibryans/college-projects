...
class Tp02q08 {
    ...
    public static void main(String[] args) {
        ...
        do {
            ...
            if (isNotEnd(entry)) {
                // faz a pesquisa
                boolean encontrado = lista.pesquisaBinaria(entry);
...
class Lista {
    ...
    public boolean pesquisaBinaria(String nome) {
        return pesquisaBinaria(nome, 0, n-1);
    }

    // Pesquisa binária recursivo
    private boolean pesquisaBinaria(String nome, int esq, int dir) {
        boolean result = false;

        if (esq <= dir) {
            int meio = (esq+dir)/2;

            switch(nome.compareTo(personagens[meio].getNome())) {
                // Se forem iguais, achou o resultado
                case 0: result = true;
                        break;  

                // Se o nome for menor que o nome do personagem[meio], pesquisa entre esq e meio
                case 1: result = pesquisaBinaria(nome, esq, meio-1); 
                        break;

                // Se o nome for maior, pesquisa na metade entre meio e dir
                case 2: result = pesquisaBinaria(nome, meio+1, dir); 
                        break;
            }
        }

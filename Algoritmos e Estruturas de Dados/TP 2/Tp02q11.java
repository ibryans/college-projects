...
class Lista {
    ...
    /**
     * Ordena o array de personagens por inseração (anoNascimento)
     */
    public void ordenarPorInsercao() {
        for (int i = 1; i < n; i++) {
            Personagem temp = personagens[i];
            int j = i-1;
            while(j >= 0 && personagens[j].getAnoNascimento().compareTo(temp.getAnoNascimento()) > 0) {

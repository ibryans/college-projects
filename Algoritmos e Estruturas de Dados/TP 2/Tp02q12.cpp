...
void insercaoPorCor(int cor, int h){
   for (int i = (h + cor); i < n; i+=h) {
      ...
      int j = i - h;
      while ((j >= 0) && (lista[j].peso*10 > tmp.peso*10)) {
         lista[j + h] = lista[j];
         j-=h;
      }
      lista[j + h] = tmp;
...
void desempate() {
   int pesoEmpatado = lista[0].peso*10;
   int i = 0;
   int j = 0;
   while (i < n) {
      pesoEmpatado = lista[j].peso*10;
      while (j < n && lista[j].peso*10 == pesoEmpatado) {
         j++;
      }
...
int main(int argc, char const *argv[]) {
   ...
   // desempata os personagens com mesmo peso (por nome)
   desempate();

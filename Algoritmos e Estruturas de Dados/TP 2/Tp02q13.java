...
class Lista {
    ...
    public void constroi(int tamanho){
        for(int i = tamanho; i > 1 && personagens[i].getAltura() > personagens[i/2].getAltura(); i /= 2){
            swap(i, i/2);
        }
    }
    ...
    public void reconstroi(int tamanho){
        ...
        while (i <= (tamanho/2)) {
           ...
           if (this.personagens[2*i].getAltura() > this.personagens[2*i+1].getAltura() || 2*i == tamanho) {
                filho = 2*i;
           } else {
                filho = 2*i + 1;
           }
  
           if (this.personagens[i].getAltura() < this.personagens[filho].getAltura()){
              swap(i, filho);
              i = filho;
           } else {
              i = tamanho;
    ...
    public void heapsort() {
        ...
        for(int i = 0; i < n; i++){
            tmp[i + 1] = this.personagens[i];
        }

        this.personagens = tmp;
        ...
        tmp = this.personagens;
        this.personagens = new Personagem[n];
        for(int i = 0; i < n; i++){
            this.personagens[i] = tmp[i+1];
      }
    }
    ...
    public void desempataPorInsercao(int comeco, int limite) {
        for (int i = comeco+1; i < limite; i++) {
            ...
            int j = i-1;
            while(j >= 0 && personagens[j].getNome().compareTo(temp.getNome()) > 0) {
    ...
    public void desempate() {
        ...
        int j = 0;
        while (i < n) {
            alturaEmpatada = personagens[j].getAltura();
            while (j < n && personagens[j].getAltura() == alturaEmpatada) {
                j++;
            }

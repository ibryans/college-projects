...
/** Troca duas células lugar (muda somente o elemento) **/
...
/** Método utilizado para pegar o pivo de um determinado intervalo (elemento do meio) **/
Personagem pegarPivo(CelulaDupla *esq, CelulaDupla *dir) {
   int tam = 0;
   int i = 0;
   CelulaDupla *celulaPivo;
   for(CelulaDupla *tmp = esq; tmp != dir; tmp = tmp->prox, tam++);
   for (celulaPivo = esq; i < tam; celulaPivo = celulaPivo->prox, i++);

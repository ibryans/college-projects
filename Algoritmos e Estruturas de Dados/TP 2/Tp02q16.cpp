...
void desempataPorInsercao(int comeco, int limite) {
   for (int i = comeco+1; i < limite; i++) {
      ...
      int j = i-1;
      while(j >= comeco && strcmp(lista[j].nome, temp.nome) > 0) {
            lista[j+1] = lista[j];
            j--;
      }
      lista[j+1] = temp;
...
void desempate() {
   char *anoEmpatado = (char*) malloc (sizeof(char));
   int i = 0;
   int j = 0;
   while (i < n) {
      anoEmpatado = lista[j].anoNascimento;
      while (j < n && strcmp(lista[j].anoNascimento, anoEmpatado)==0) {
            j++;
      }

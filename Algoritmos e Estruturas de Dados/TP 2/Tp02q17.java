...
class Lista {
    ...
    public void mergesort(int esq, int meio, int dir)  { 
        // Encontrando o tamanho dos subarrays
        int n1 = meio - esq + 1; 
        int n2 = dir - meio; 
  
        Personagem tmpEsq[] = new Personagem[n1]; 
        Personagem tmpDir[] = new Personagem[n2]; 
  
        // Copiando os elementos
        for (int i = 0; i < n1; ++i) 
            tmpEsq[i] = personagens[esq + i]; 
        for (int j = 0; j < n2; ++j) 
            tmpDir[j] = personagens[meio + 1 + j]; 
        ...
        // Fazendo a ordenação
        while (i < n1 && j < n2) { 
            if (tmpEsq[i].getHomeworld().compareTo(tmpDir[j].getHomeworld()) < 0) { 
                personagens[k] = tmpEsq[i]; 
                i++; 
            } else { 
                personagens[k] = tmpDir[j]; 
                j++;
            } 
            k++; 
        } 
  
        /* Copiando o resto dos elementos de tmpEsq */
        while (i < n1) { 
            personagens[k] = tmpEsq[i];
            i++;
            k++;
        ...
        /* Copiando o resto dos elementos de tmpDir */
        while (j < n2) { 
            personagens[k] = tmpDir[j]; 
            j++;
            k++;
    ...
    void mergesort(int esq, int dir) { 
        if (esq < dir) { 
            ...
            // Pega as 2 metades do array
            mergesort(esq, meio); 
            mergesort(meio+1, dir); 
  
            // Ordenad as 2 partes
            mergesort(esq, meio, dir);
        } 
    }
    ...
    public void desempate() {
        String homeWorldEmpatado;
        int i = 0;
        int j = 0;
        while (i < n) {
            homeWorldEmpatado = personagens[j].getHomeworld();
            while (j < n && personagens[j].getHomeworld().compareTo(homeWorldEmpatado) == 0) {
                j++;
            }

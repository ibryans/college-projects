...
class Tp02q19 {
    ...
    public static void main(String[] args) {
        ...
        lista.ordenaParcialmentePorSelecao(k);
...
class Lista {
    ...
    /**
     * Ordena o array de personagens por seleção
     */
    public void ordenaParcialmentePorSelecao(int k) {
        for (int i = 0; i < k; i++) {
            int menor = i;
            for (int j = i+1; j < n; j++) {
                if (personagens[j].getNome().compareTo(personagens[menor].getNome()) < 0) {
                    menor = j;
                }
            }
            swap(menor, i);

...
class Lista {
    ...
    public void quicksortParcial(int esq, int dir, int k) {
        int i = esq, j = dir;
        Personagem pivo = personagens[(esq+dir)/2];
        while (i <= j) {
            while (personagens[i].getCorDoCabelo().compareTo(pivo.getCorDoCabelo()) < 0)
                i++;
            while (personagens[j].getCorDoCabelo().compareTo(pivo.getCorDoCabelo()) > 0)
                j--;
            if (i <= j) {
                swap(i, j);
                i++;
    ...
    public void quicksortParcial(int k) {
    ...
    void desempate() {
        ...
        int j = 0;
        while (i < n) {
            corEmpatada = personagens[j].getCorDoCabelo();
            while (j < n && personagens[j].getCorDoCabelo().compareTo(corEmpatada) == 0) {
                    j++;
            }
...
class Personagem {
    ...
    public void imprimir() {
        ...
        MyIO.print(" ## " + this.getCorDoCabelo());
